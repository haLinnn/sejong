...
class RecommendationService():
    ...
    def load_data(self):
        ...
        # 문장 리스트
        sentences = self.df['classification'].tolist()

        # TF-IDF 벡터화
        self.tfidf_vectorizer = TfidfVectorizer()
        self.tfidf_matrix = self.tfidf_vectorizer.fit_transform(sentences)
        ...
        with open('similarities_sb.pkl', 'rb') as f:
            self.similarities_sb = pickle.load(f)

        # 최종 유사도, sentence-bert 문장 유사도 행렬과 분류 코사인 유사도 행렬을 더함
        self.combined_matrix = self.similarities_sb + self.cosine_sim


    def find_similar_sentences(self, title, top_n=20):
        title_data = self.df[self.df['title'] == title]
        title_index = title_data.index.values[0]

        # 해당 작품과 다른 작품 간의 유사도
        title_similarity_scores = self.combined_matrix[title_index]
        ...
        # 유사한 작품 데이터 프레임 생성
        similar_titles = self.df.iloc[similar_indexes]
